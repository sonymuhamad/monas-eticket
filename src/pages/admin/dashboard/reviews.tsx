...
const AdminReviews: NextPageWithLayout<Props> = ({ data }: Props) => {
  ...
  const [editableReview, setEditableReview] = useState<ReviewProps | undefined>(
    undefined
  );
  // when admin click edit
  // editable review will be filled by review object, and then used in modal.

  const [idEditableReview, setIdEditableReview] = useState<number | undefined>(
    undefined
  );
  ...
  const [avatarEditableReview, setAvatarEditableReview] = useState<File | null>(
    null
  );
  ...
  const [avatarEditableUrl, setAvatarEditableUrl] = useState<
    string | undefined
  >(undefined);

  const [willBeDeletedReview, setWillBeDeletedReview] = useState<
    number | undefined
  >(undefined);

  const [nameAddedReview, setNameAddedReview] = useState<string>("");
  const [textAddedReview, setTextAddedReview] = useState<string>("");
  const [avatarAddedReview, setAvatarAddedReview] = useState<undefined | File>(
    undefined
  );
  const [shownAddedReview, setShownAddedReview] = useState(false);
  const [avatarAddedObjectUrl, setAvatarAddedObjectUrl] = useState<
    string | undefined
  >(undefined);
  ...
  const setReviewsAfterDelete = (idDeletedReview: number) => {
    setReviews((prev) =>
      prev.filter(({ id_review }) => id_review !== idDeletedReview)
    );
  };
  ...
  const handleSubmitEditReview = async (e: React.SyntheticEvent) => {
    ...
    try {
      ...
      const { ok, updatedData }: { ok: boolean; updatedData: ReviewProps } =
        await res.json();
  ...
  const handleSubmitCreateReview = async (e: React.SyntheticEvent) => {
    ...
    try {
      ...
      const { ok, createdReview }: { ok: boolean; createdReview: ReviewProps } =
        await res.json();
      if (ok) {
    ...
    <>
      ...
      <Grid container maxWidth={"xl"} spacing={2}>
        {filteredReviews.map((review) => {
          ...
          return (
            <Grid item md={4} sm={6} xs={12} key={id_review}>
              <Box
                component={Paper}
                elevation={1}
                sx={{
                  ...
                  >
                    <Avatar
                      ...
                      src={
                        createObjectURL
                          ? createObjectURL
                          : `/review-images/${avatarEditableUrl}`
                      }
                      sx={{ width: 56, height: 56 }}
                    />
                  </Badge>
                    ...
                    <FormControlLabel
                      control={
                        <Checkbox
                          ...
                          onChange={(e) =>
                            setShownEditableReview(e.target.checked)
                          }
                        />
                      }
                      label="Shown"
        ...
        <Modal open={openAddModal} onClose={handleCloseAddModal}>
          ...
          >
            <Box component="form" onSubmit={handleSubmitCreateReview}>
              <Grid
                display="flex"
                alignItems={"center"}
                justifyContent={"center"}
...
const LoadingSkeletons = () => {
  return (
    <Grid container spacing={2}>
      <Grid md={4} sm={6} xs={12} item justifyContent={"center"}>
...
AdminReviews.getLayout = (reviewPage) => {
  return (
    <LayoutAdmin>
      <Suspense fallback={<LoadingSkeletons />}>{reviewPage}</Suspense>
    </LayoutAdmin>
  );
};
